"""강좌별 출석 회차와 회원 출석 기록 SQLAlchemy 모델."""
...
class AttendanceSession(TimestampVersionMixin, Base):
    ...
    sequence_no: Mapped[int] = mapped_column(Integer, default=1, server_default="1", nullable=False)
...
class AttendanceRecord(TimestampVersionMixin, Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "status IN ('present', 'absent', 'late', 'excused')", name="status_allowed"
        ),
        CheckConstraint("version >= 1", name="version_positive"),
