"""강좌 기준정보, 회차별 개설과 복수 시간표 SQLAlchemy 모델."""
...
class Course(TimestampVersionMixin, Base):
    ...
    category_id: Mapped[int] = mapped_column(
        ForeignKey("course_categories.id", ondelete="RESTRICT"), nullable=False
    )
...
class Term(TimestampVersionMixin, Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("max_registrations_per_member >= 0", name="registration_limit_nonnegative"),
        CheckConstraint(
            "status IN ('draft', 'open', 'closed', 'finalized')", name="status_allowed"
        ),
        CheckConstraint("version >= 1", name="version_positive"),
    )
...
class CourseOffering(TimestampVersionMixin, Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "status IN ('draft', 'open', 'closed', 'cancelled')", name="status_allowed"
        ),
        CheckConstraint("version >= 1", name="version_positive"),
...
class CourseSchedule(Base):
    ...
    offering_id: Mapped[int] = mapped_column(
        ForeignKey("course_offerings.id", ondelete="CASCADE"), nullable=False
    )
