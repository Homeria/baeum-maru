"""사용자, 접속 코드와 로그인 세션 SQLAlchemy 모델."""
...
class User(TimestampVersionMixin, Base):
    __tablename__ = "users"
    __table_args__ = (
        CheckConstraint("role IN ('staff', 'temporary_staff', 'viewer')", name="role_allowed"),
...
class AccessCode(TimestampVersionMixin, Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "revoked_at IS NULL OR revoked_at >= issued_at", name="revoked_after_issue"
        ),
        CheckConstraint("hidden_at IS NULL OR hidden_at >= issued_at", name="hidden_after_issue"),
        CheckConstraint("version >= 1", name="version_positive"),
        Index("ix_access_codes_user_id", "user_id"),
        Index("ix_access_codes_lifecycle", "expires_at", "revoked_at", "hidden_at"),
    )
...
class UserSession(Base):
    ...
    __table_args__ = (
        CheckConstraint("expires_at > issued_at", name="expiry_after_issue"),
        CheckConstraint("last_seen_at >= issued_at", name="last_seen_after_issue"),
        CheckConstraint(
            "revoked_at IS NULL OR revoked_at >= issued_at", name="revoked_after_issue"
        ),
    ...
    access_code_id: Mapped[int] = mapped_column(
        ForeignKey("access_codes.id", ondelete="RESTRICT"), nullable=False
    )
