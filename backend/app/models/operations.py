"""감사 로그, 장기 작업, 멱등성 기록과 작업 잠금 SQLAlchemy 모델."""
...
class AuditLog(Base):
    ...
    __table_args__ = (
        CheckConstraint("actor_kind IN ('user', 'launcher', 'system')", name="actor_kind_allowed"),
...
class OperationJob(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("success_count + failure_count <= total_count", name="counts_within_total"),
...
class OperationJobError(Base):
    ...
    __table_args__ = (
        CheckConstraint("row_number IS NULL OR row_number >= 1", name="row_number_positive"),
        Index("ix_operation_job_errors_job_id", "job_id"),
    )

    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    job_id: Mapped[int] = mapped_column(
        ForeignKey("operation_jobs.id", ondelete="CASCADE"), nullable=False
    )
...
class OperationLock(Base):
    ...
    __table_args__ = (
        CheckConstraint("expires_at > acquired_at", name="expiry_after_acquire"),
