"""강좌 신청의 현재 상태와 상태 변경 이력 SQLAlchemy 모델."""
...
class Registration(TimestampVersionMixin, Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(f"status IN ({REGISTRATION_STATUSES})", name="status_allowed"),
        CheckConstraint(
    ...
    status: Mapped[str] = mapped_column(
        String(16), default="applied", server_default="applied", nullable=False
    )
...
class RegistrationStatusHistory(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(f"to_status IN ({REGISTRATION_STATUSES})", name="to_status_allowed"),
