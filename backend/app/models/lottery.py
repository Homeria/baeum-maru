"""재현 가능한 추첨 실행, 대상 snapshot과 결과 SQLAlchemy 모델."""
...
class LotteryRun(Base):
    ...
    status: Mapped[str] = mapped_column(
        String(16), default="prepared", server_default="prepared", nullable=False
    )
...
class LotteryRunTarget(Base):
    ...
    lottery_run_id: Mapped[int] = mapped_column(
        ForeignKey("lottery_runs.id", ondelete="CASCADE"), nullable=False
    )
...
class LotteryResult(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("result IN ('selected', 'waitlisted', 'rejected')", name="result_allowed"),
    ...
    lottery_run_target_id: Mapped[int] = mapped_column(
        ForeignKey("lottery_run_targets.id", ondelete="CASCADE"), nullable=False
    )
    registration_id: Mapped[int] = mapped_column(
        ForeignKey("registrations.id", ondelete="RESTRICT"), nullable=False
    )
