...
def list_terms() -> list[dict[str, Any]]:
    return course_repo.list_terms()


def get_term(term_id: int) -> dict[str, Any]:
...
def create_term(
    *,
    ...
    return course_repo.create_term(
        name=name,
        starts_on=starts_on,
        ends_on=ends_on,
        registration_opens_at=registration_opens_at,
        registration_closes_at=registration_closes_at,
        max_registrations_per_member=max_registrations_per_member,
        status=status,
    )
...
def delete_term(term_id: int) -> None:
    if not course_repo.delete_term(term_id):
        raise ResourceNotFoundError("term_not_found", "학기를 찾을 수 없습니다.")
