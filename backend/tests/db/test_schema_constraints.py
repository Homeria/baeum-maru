...
def test_identity_and_reference_checks_reject_invalid_values(migrated_engine: Engine) -> None:
    ...
    for statement in invalid_statements:
        assert_integrity_error(migrated_engine, statement)


def test_course_capacity_schedule_and_section_constraints(migrated_engine: Engine) -> None:
    seed_operational_graph(migrated_engine)
...
def test_registration_lottery_and_attendance_constraints(migrated_engine: Engine) -> None:
    seed_operational_graph(migrated_engine)
    ...
    with migrated_engine.begin() as connection:
        connection.execute(
            text("INSERT INTO registrations (id, member_id, offering_id) VALUES (2, 2, 1)")
        )
...
def test_job_idempotency_and_lock_checks_reject_invalid_state(migrated_engine: Engine) -> None:
    invalid_statements = (
