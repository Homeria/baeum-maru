...
- [ ] 회원/강좌/신청/추첨/Excel 한 cycle integration test
...
- [ ] launcher bridge가 bundled launcher 자산 이외의 경로에서 호출되지 않음
...
- [ ] WebSocket 수신 시 관련 데이터만 갱신되고 입력 중 form을 덮어쓰지 않음
- [ ] WebSocket 인증, heartbeat, disconnect 정리와 재연결 후 query 재검증이 동작함
- [ ] 한글, 날짜/시간, 전화번호 입력이 Windows browser에서 정상 동작

## 호스트 런처

- [ ] 실행 직후 업무 서버가 정지 상태임
- [ ] 기본 브라우저 없이 pywebview 독립 창이 열리고 WebView2 누락 시 복구 안내가 표시됨
- [ ] 시작/중지/재시작과 중복 command 방지 확인
- [ ] 실제 내부망 IPv4 주소와 접속 URL이 명확히 표시됨
- [ ] 접속 코드, 로그, 백업, 설정 기능 확인
- [ ] 원격 navigation과 개발자 도구가 production launcher에서 차단됨
...
- [ ] 역할 권한이 API와 WebSocket 모두에서 강제됨
...
- [ ] network 단절, server 재시작, WebSocket 재연결, backup/restore scenario 확인
