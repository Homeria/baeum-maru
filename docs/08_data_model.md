...
이 문서는 Python 프로토타입의 단일 초기 SQLAlchemy/Alembic 스키마 명세다. `feat/sqlalchemy-alembic-baseline`은 이 문서를 그대로 모델과 첫 migration으로 구현한다.
...
다음 브랜치에서는 임의로 컬럼을 축약하거나 과거 Go schema를 복원하지 않고 이 기준선을 SQLAlchemy 2 model과 단일 Alembic revision으로 옮긴다.
