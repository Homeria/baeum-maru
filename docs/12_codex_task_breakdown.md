...
- 여러 table을 바꾸는 use case는 해당 작업을 소유한 Repository 공개 함수 하나가 같은 connection에서 수행하고 commit 또는 rollback한다.
- Router는 Pydantic 입력을 풀어 service에 전달하며 service는 `app.schemas`, Pydantic, `sqlite3`, `app.db`를 import하지 않는다.
