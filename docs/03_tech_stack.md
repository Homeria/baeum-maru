...
- 설치 없이 포터블 실행 가능
- `exe` 더블클릭으로 네이티브 컨트롤 패널 실행
- 컨트롤 패널에서 내부 웹서버 시작/중지
...
```text
Language: Go
Desktop Launcher GUI: Fyne
HTTP Server: net/http + chi
Database: SQLite
DB Access: database/sql
SQLite Driver: modernc.org/sqlite 또는 mattn/go-sqlite3
...
Fyne은 Go 기반 GUI 툴킷이다.  
배움마루에서는 전체 업무 UI가 아니라 **네이티브 런처 컨트롤 패널**을 위해 사용한다.
