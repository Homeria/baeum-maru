...
아직 실사용 데이터가 없는 개발 단계이므로, 기존 DB 파일을 이어서 쓰기 위한 호환 migration은 기본적으로 작성하지 않는다.

- 최신 기준 스키마는 `internal/migration/sql/001_init.sql`에 직접 반영한다.
- 스키마가 크게 바뀌면 기존 로컬 DB 파일을 삭제하고 새로 만든다.
- 사용자가 명시적으로 기존 DB를 이어서 쓰겠다고 요청한 경우에만 별도 migration을 작성한다.
